...
const FindProduct = () => {
  const { countryList, cityList, countryAndCityLoading, selectedCity } =
  ...
  return (
    <div className="bg-white flex items-center relative  flex-col gap-y-4 lg:px-20 lg:py-10 p-4 xl:w-[70%] w-11/12 rounded-2xl">
      ...
      <div className="flex md:flex-row flex-col w-full items-center justify-between gap-4 md:px-1.5 md:py-1.5 p-3 border border-opacity-50 border-borderColor md:rounded-[2rem] rounded-2xl bg-gray-50">
        ...
        <div
          className={`flex group relative items-center cursor-pointer gap-3 lg:w-2/12 md:w-3/12 w-full`}
        >
          ...
          />
          <div
            className={`absolute group-hover:scale-100 scale-0 transition-all origin-top bg-white min-w-[13rem] text-left ease-in-out duration-300 top-9 left-0 p-3 max-h-72 overflow-y-auto custom_scrollbar rounded-2xl shadow-2xl text-textColor space-y-2 `}
          >
